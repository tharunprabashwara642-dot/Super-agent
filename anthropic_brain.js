    ...
    try {
      ...
      if (!resp.ok) {
        ...
        if (status === 429 || status === 529 || (status >= 500 && status < 600)) {
          ...
          console.error(`⚠️ Key #${(cursor - 1) % apiKeys.length} transient error ${status} (${msg}) — rotating.`);
          continue;
        }
      ...
      if (e.name === "AbortError") {
        return { error: { message: `Request timed out after ${timeoutMs}ms.` } };
      }
      console.error(`⚠️ Key #${(cursor - 1) % apiKeys.length} request error (${e.message}) — rotating.`);
      continue;
    }
  }
