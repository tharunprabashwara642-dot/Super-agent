...
// ============================================================
// Swapped to gemini_brain.js: no money for Anthropic right now, so the
// text/tool-calling brain runs on a free Gemini key instead. Same
// exported shape (chatShimmed/keyCount/addKeyToPool/setUsageCallback),
// so nothing else in this file needed to change. Flip back to
// "./anthropic_brain" here (one line) whenever there's budget for it.
...
async function deleteCustomTool(name) {
  ...
  // (FIX) Same silent-no-op class as forgetMemory: delete().eq() reports
  // error:null even for a name that never existed, and the in-memory
  // unregister always "succeeds" (deleting a key that isn't there is a
  // no-op), so a typo'd tool name previously came back deleted:true.
  const { data, error } = await supabase.from("agent_custom_tools").delete().eq("name", name).select("name");
  if (error) return { deleted: false, name, reason: error.message };
  if (!data || data.length === 0) return { deleted: false, name, reason: "No custom tool with that name — nothing was deleted." };
...
async function forgetMemory(memoryId) {
  ...
  // (FIX) Supabase's delete().eq() returns error:null even when the id
  // didn't match any row — so a wrong/already-deleted memory_id was being
  // reported back as deleted:true with nothing actually removed. Chaining
  // .select() makes Postgrest return the deleted rows, so we can tell "0
  // rows matched" apart from "deleted successfully" instead of guessing.
  const { data, error } = await supabase.from("agent_memories").delete().eq("id", memoryId).select("id");
  if (error) return { deleted: false, reason: error.message };
  if (!data || data.length === 0) return { deleted: false, reason: "No memory found with that id — nothing was deleted." };
  return { deleted: true };
}
...
async function updateMemory(memoryId, newContent) {
  ...
  // (FIX) Same issue as forgetMemory — update().eq() alone can't tell "0
  // rows matched" from "1 row updated", both come back as error:null.
  const { data, error } = await supabase.from("agent_memories").update({ content: newContent, embedding }).eq("id", memoryId).select("id");
  if (error) return { updated: false, reason: error.message };
  if (!data || data.length === 0) return { updated: false, reason: "No memory found with that id — nothing was updated." };
  return { updated: true };
}
...
async function executeFunctionCall(fc, goalContext) {
  try {
    ...
    // (FIX) This used to send its own "Working on action..." / "Completed
    // action..." bot.sendMessage() call here, on top of whichever caller
    // invoked us. All three call sites (direct chat handler, goal-step
    // runner, autonomous tick) already run their own single-message
    // renderStatus()/editMessageText loop around this function — so every
    // tool call was producing that ONE edited status line PLUS two brand
    // new separate messages (start + done) from here, and a third on
    // error. That's exactly the "spam new messages instead of editing one"
    // failure Boss flagged. Progress reporting now lives in exactly one
    // place per call site (their renderStatus/toNarrativeLine), and this
    // function just executes the tool and returns the real result — no
    // Telegram side effects of its own.
...
async function callBrain(contents, systemInstruction) {
  ...
  // 150s instead of the 60s default — a heavier request (long context,
  // several tools in play, a "-thinking"/high-reasoning-effort model)
  // can genuinely take longer than 60s to answer, especially through a
  // reseller/proxy endpoint like tabitoken. Website-gen calls already
  // use 180s (see nvidiaChatShimmed(... , 180000) below) for the same
  // reason.
  const data = await nvidiaChatShimmed(contents, systemInstruction, combinedTools, null, 150000);
...
async function handleChatMessage(userText) {
  if (brainKeyCount() === 0) {
    return "⚠️ No Anthropic API keys configured. Set ANTHROPIC_API_KEY in Railway → Variables.";
  }
...
${JSON.stringify({
    ...
    // this turn, regardless of round. The model's final text reply used to
    // be the ONLY thing sent back to the user (see the `return textReply`
    // below) — nothing checked that a link it claimed to include ("🔗
    // Download ...") was an actual URL a tool returned. A low-effort model
    // (or a flaky custom tool that silently no-ops) can then confidently
    // narrate "ඉවරයි, මෙන්න ලින්ක් එක" with no real link behind it at all.
    // Collecting every real url/link a tool call actually returns here, and
    // force-appending whichever of them the model's text didn't already
    // include verbatim, makes the delivered link deterministic instead of
    // dependent on the model faithfully relaying it.
    const collectedLinks = [];
    function collectLinksFromResult(toolName, result) {
      const link = result?.url || result?.link || result?.download_url || result?.file_url;
      if (link && !collectedLinks.some((l) => l.link === link)) {
        collectedLinks.push({ tool: toolName, link });
      }
    }
    ...
    try {
      for (let i = 0; i < MAX_TOOL_ROUNDS; i++) {
        ...
        if (functionCalls.length === 0) {
          ...
          // Force-append any real link a tool returned this turn that the
          // model's own text didn't already quote verbatim — see
          // collectLinksFromResult above. This is what actually fixes "the
          // bot said 🔗 done but there was no real link": the delivered
          // link no longer depends on the model choosing to relay it.
          let finalReply = textReply || "I processed your request but have no response.";
          const missingLinks = collectedLinks.filter((l) => !finalReply.includes(l.link));
          if (missingLinks.length > 0) {
            finalReply += "\n\n" + missingLinks.map((l) => `🔗 ${l.link}`).join("\n");
          }
          return finalReply;
        }
        ...
        for (const fc of functionCalls) {
          ...
          collectLinksFromResult(fc.name, result);
          responseParts.push({ functionResponse: { name: fc.name, response: { result } } });
        }
        contents.push({ role: "user", parts: responseParts });
...
async function checkScheduledTasksInner() {
  ...
  for (const task of dueTasks) {
    ...
    try {
      ...
      await sendLongMessage(CHAT_ID, isDigest ? result : `${icon} ${result}`); // was bot.sendMessage — could hit Telegram's 4096-char cap
...
Decide if you should act now.`;
    ...
    // (NEW) executeFunctionCall no longer sends its own per-tool Telegram
    // messages (that was duplicate spam across every call site — see the
    // fix in executeFunctionCall). This path was the one caller that had
    // NO status UI of its own at all, so removing that made autonomous-
    // tick tool calls invisible in Telegram until either a final text
    // reply or a confirmation button showed up — a real regression for an
    // unattended run the user can't see into otherwise. Give it the same
    // single-message, edited-in-place narrator the direct chat handler
    // uses: one message, edited per step, never a new message per tool.
    let autoStatusMsgId = null;
    let autoStatusLines = [];
    async function renderAutoStatus(done) {
      const shown = autoStatusLines.slice(-14);
      const header = done ? `✅ Boss, autonomous check එකේ වැඩේ ඉවරයි!` : `🌙 Boss, autonomous check එකේදී මන් වැඩක් කරගෙන යනවා...`;
      const text = `${header}\n` + shown.join("\n");
      try {
        if (autoStatusMsgId === null) {
          const sent = await bot.sendMessage(CHAT_ID, text);
          autoStatusMsgId = sent.message_id;
        } else {
          await bot.editMessageText(text, { chat_id: CHAT_ID, message_id: autoStatusMsgId });
        }
      } catch (e) {
        // never let a status-message hiccup break the actual tool run
      }
    }
    ...
    try {
      for (let i = 0; i < 4; i++) {
        ...
        for (const fc of functionCalls) {
          const label = HUMAN_TOOL_LABELS[fc.name] || fc.name.replace(/_/g, " ");
          const lineIdx = autoStatusLines.length;
          autoStatusLines.push(`▫️ ${label}... පටන් ගත්තා`);
          await renderAutoStatus(false);

          const result = await executeFunctionCall(fc);
          console.log("Autonomous tool call:", fc.name, JSON.stringify(result));

          const tag = toolOutcomeTag(result);
          autoStatusLines[lineIdx] = tag.startsWith("✅")
            ? `▫️ ${label} — ✅ හරි ගියා`
            : tag.startsWith("⏸️")
            ? `▫️ ${label} — ⏸️ confirm එකක් ඕනේ`
            : `▫️ ${label} — ⚠️ ${tag.replace("⚠️", "").trim()}`;
          await renderAutoStatus(false);
      ...
      if (autoStatusMsgId !== null) await renderAutoStatus(true);
    }
  } catch (e) {
...
// exactly what httpServer.listen() below provides (it doubles as
// Railway's health check).

const http = require("http");
const { WebSocketServer } = require("ws");
const { GoogleGenAI: VoiceGenAI, Modality: VoiceModality } = require("@google/genai");
...
const VOICE_RELAY_PORT = process.env.PORT || 3000;
httpServer.listen(VOICE_RELAY_PORT, () => {
  console.log(`🎙️ Voice relay + health server listening on port ${VOICE_RELAY_PORT}`);
...
// STARTUP
// ============================================================
console.log(`🚀 Night Agent started with ${brainKeyCount()} Gemini text key(s) (+ ${API_KEYS.length} Gemini key(s), voice-only)`);
