...
try {
  bot = new TelegramBot(TELEGRAM_BOT_TOKEN, { 
    polling: {
      ...
      params: {
        timeout: 10,
        // Explicit here (not just via the startPolling prototype patch in
        // telegram_bootstrap_patch.js) because autoStart:true may invoke
        // the library's internal polling init through a path that never
        // calls the patched startPolling(). Telegram remembers the last
        // allowed_updates value used for this bot token — if it was ever
        // narrowed to just "message" (e.g. an older deploy, or a getUpdates
        // call with no allowed_updates at all defaulting to all-but-none-
        // explicit), callback_query stops arriving with NO error on either
        // side: the button just does nothing and nothing gets logged,
        // because the event never reaches this process at all.
...
// Diagnostic: log the type of every raw update the moment it arrives from
// Telegram, before any handler logic runs. If a button tap never produces
// a "raw update: callback_query" line here, the event isn't reaching this
// process at all (allowed_updates / multiple-poller issue) — that's a
// different problem than the callback_query handler itself misbehaving,
// and this line is what tells the two apart from Railway logs alone.
bot.on('callback_query', (q) => {
  console.log(`📥 raw update: callback_query id=${q.id} data=${q.data}`);
});
...
// Sub-agent role definitions/prompts (researcher, coder, writer, verifier,
// planner). Wired into the dispatch_sub_agent tool below via runToolDirectly.
const subAgents = require("./sub_agents");
...
const CHAT_TOOLS = [
  {
    functionDeclarations: [
      ...
      {
        name: "sandbox_run",
        description: "Self-testing sandbox terminal. Write candidate code files into an isolated throwaway directory and run a command against them (defaults to `node --check` on the first .js file). Use this to TEST code you're about to ship BEFORE calling edit_own_code/insert_own_code: write it here, run it, read stdout/stderr/exit_code, and if it's buggy fix and re-run until it passes. Runs unattended (no confirmation) so you can iterate in the background. Confined to the sandbox dir; only works if AGENT_ENABLE_SANDBOX=true. WARNING: files written here stay on a private server disk — the user CANNOT see or download them, and there is no automatic delivery step. Never use this tool to \"generate and send\" a deliverable (PDF, document, etc.) for the user; use generate_mcq_pdf or another delivery-capable tool for that, and never tell the user a file was sent unless a tool actually returned delivered:true.",
        parameters: {
          type: "OBJECT",
          properties: {
            files: {
      ...
      },
      {
        ...
        description: "Generates a real exam-style model-paper PDF on any given subject/topic and DELIVERS it directly to the user as a Telegram document attachment — this is the only tool that actually sends a finished PDF to the chat. Supports MCQ (default), structured (sub-parts a/b/c with marks + marking scheme), and essay question types for any subject — not limited to ICT. Do NOT use sandbox_run to \"generate a PDF\": sandbox_run only writes files to a private throwaway disk that the user can never see or download. Use this tool instead whenever the user asks for an MCQ / structured / essay paper or model paper PDF.",
        parameters: {
          type: "OBJECT",
          properties: {
            subject: { type: "STRING", description: "The exam subject, e.g. \"Biology\", \"Combined Maths\", \"ICT\". Defaults to whatever is given in `topic` if omitted." },
            topic: { type: "STRING", description: "Specific topic/syllabus area within the subject, e.g. \"Cell Division\"." },
            question_type: { type: "STRING", description: "One of \"mcq\", \"structured\", or \"essay\". Defaults to \"mcq\"." },
            count: { type: "NUMBER", description: "Number of questions. Defaults to 50 for mcq, 8 for structured, 4 for essay." },
            language: { type: "STRING", description: "Language for the questions, e.g. \"Sinhala\" or \"English\". Defaults to Sinhala." },
            title: { type: "STRING", description: "Title for the PDF. Defaults to \"<subject> <topic> <Question type> Model Paper\"." },
          },
          required: ["topic"],
        },
      },
      {
        ...
        description: "Spawns a focused sub-agent (researcher, coder, writer, verifier, or planner) to work on one narrow piece of a larger task and returns its result as text. Use this to delegate a sub-task instead of trying to do everything in one pass — e.g. dispatch a researcher to gather facts before writing, or a verifier to double-check a claimed result.",
        parameters: {
          type: "OBJECT",
          properties: {
            role: { type: "STRING", description: "One of \"researcher\", \"coder\", \"writer\", \"verifier\", \"planner\". If omitted, the role is auto-picked from the task text." },
            task: { type: "STRING", description: "The specific sub-task for this sub-agent to do." },
            context: { type: "STRING", description: "Optional extra context: the parent goal or background the sub-agent needs." },
          },
          required: ["task"],
        },
      },
...
// ---- persist pending confirmations to Supabase ----
// Root-cause fix for "I tap Yes/No and nothing happens": pendingConfirmations
// used to live ONLY in this process's memory. If the container restarted
// (crash loop, redeploy, Railway sleep/wake) in the window between the
// button being sent and being tapped, the new process started with an
// empty array — the button still looked live in Telegram, but the tap had
// nothing to resolve against, so callback_query's idx===-1 branch fired
// silently (a small toast the user could easily miss, no chat message).
// Mirroring every queue/resolve to the DB means a tap after a restart is
// recovered from there instead of failing quietly.
async function persistPendingConfirmation(pc) {
  try {
    await supabase.from("pending_confirmations").upsert({
      id: pc.id,
      tool_name: pc.toolName || null,
      args: pc.args || {},
      description: pc.description,
      kind: pc.kind || null,
      payload: pc.payload || null,
      goal_id: pc.goalId || null,
      step_id: pc.stepId || null,
      goal_title: pc.goalTitle || null,
      message_id: pc.messageId || null,
    });
  } catch (e) {
    console.error("persistPendingConfirmation error:", e.message);
  }
}

async function removePersistedConfirmation(id) {
  try {
    await supabase.from("pending_confirmations").delete().eq("id", id);
  } catch (e) {
    console.error("removePersistedConfirmation error:", e.message);
  }
}

// Called once at boot: reload anything still pending from a previous
// process into memory, and re-send the buttons so they're tappable again
// instead of silently stuck.
async function restorePendingConfirmationsFromDb() {
  try {
    const { data, error } = await supabase.from("pending_confirmations").select("*");
    ...
    for (const row of data) {
      pendingConfirmations.push({
        ...
        buttonsSent: false, // force a fresh, tappable button after restart
      });
    }
    console.log(`🔁 Restored ${data.length} pending confirmation(s) from a previous session — re-sending buttons.`);
    await sendConfirmationButtons();
  } catch (e) {
    console.error("restorePendingConfirmationsFromDb error:", e.message);
  }
}
...
async function sendConfirmationButtons() {
  ...
  for (const pc of pendingConfirmations) {
    ...
    try {
      ...
      // Markdown parse errors (unbalanced ` or * in a value the model
      // filled into describeAction) throw here — retry once as plain text
      // instead of dropping the button entirely.
      try {
        await bot.sendMessage(CHAT_ID, pc.description, {
          reply_markup: {
            inline_keyboard: [[
...
// Posts a sub-agent message to Telegram, tolerating Markdown parse errors
// (sub-agent output can contain stray * _ ` chars) and Telegram's ~4096
// char message limit by chunking long output.
async function postSubAgentMessage(text) {
  const chunks = [];
  let s = String(text || "");
  while (s.length > 3500) {
  ...
  for (const chunk of chunks) {
    try {
      ...
      try { await bot.sendMessage(CHAT_ID, chunk); } catch (_) {}
    }
  }
}

// (NEW) Sub-agent dispatcher — actually wires sub_agents.js in. Runs a
// single focused, tool-less generation under a role-specific system prompt
// (picked from args.role, or auto-picked from the task text) and returns
// the sub-agent's text output to the parent agent. Previously sub_agents.js
// was never required anywhere, so nothing about "sub-agents talking" was
// real — this both runs the sub-agent AND posts its task + reply to
// Telegram so the user can actually see the exchange, not just a silent
// tool result the parent model summarizes on its own.
async function dispatchSubAgent(args = {}) {
  const task = String(args?.task || "").trim();
  if (!task) return { error: true, message: "dispatch_sub_agent needs a `task`." };

  const roleKey = args?.role && subAgents.ROLES[args.role] ? args.role : subAgents.pickRole(task);
  const role = subAgents.ROLES[roleKey];
  const systemInstruction = subAgents.buildSubAgentPrompt(roleKey, args?.context || task);
  const contents = [{ role: "user", parts: [{ text: task }] }];

  await postSubAgentMessage(`${role.emoji} *${role.name} sub-agent* → ${task}`);

  const data = await nvidiaChatShimmed(contents, systemInstruction, null);
  if (data?.error) {
    const msg = data.error.message || "Sub-agent request failed.";
    await postSubAgentMessage(`${role.emoji} *${role.name}* ❌ ${msg}`);
  ...
  const text = (data?.candidates?.[0]?.content?.parts || [])
    ...
    .join("")
    .trim();
  const result = text || "(sub-agent returned no output)";

  await postSubAgentMessage(`${role.emoji} *${role.name}* කිව්වේ:\n${result}`);
...
async function runToolDirectly(name, args) {
  ...
  if (name === "generate_mcq_pdf") {
    // Puppeteer/Chromium (v3) is the only renderer with real Sinhala text
    // shaping, so it's the only one actually wired up — the old v1/v2
    // PDFKit branches never worked for Sinhala and are gone. chat_id is
    // passed explicitly so delivery always targets the right Telegram chat
    // even if the caller didn't include it in args.
    return await require("./native_document_tool_v3_browser").generateDocument({ ...(args || {}), chat_id: args?.chat_id || CHAT_ID });
  }
  if (name === "dispatch_sub_agent") return await dispatchSubAgent(args);
...
async function executeFunctionCall(fc, goalContext) {
  ...
  try {
    ...
    // Document-generation tools (native_document_tool_v3_browser.js) report
    // fine-grained progress ("generating question 12/50", "rendering PDF",
    // "uploading to Telegram"...) through a progress() callback that looks
    // for global.__nightAgentLiveBridge.step. Nothing ever set that bridge,
    // so live_activity.js's whole timer + step-list card sat unused as dead
    // code and the user only ever saw a flat ⏳/⌛ spinner instead of real
    // steps. Wire it in here for this tool specifically.
    if (fc.name === "generate_mcq_pdf") {
      const activity = require("./live_activity");
      global.__nightAgentLiveBridge = {
        step: (msg, state) => activity.step(msg, state, { chatId: CHAT_ID }),
      };
      try {
        await activity.start("🧠 ඉල්ලීම තේරුම් ගන්නවා", { chatId: CHAT_ID });
        const toolResult = await runToolDirectly(fc.name, fc.args);
        if (toolResult && (toolResult.error || toolResult.error === true)) {
          await activity.fail(String(toolResult.message ? `❌ ${toolResult.message}` : "❌ PDF එක හදන්න බැරි උනා"), { chatId: CHAT_ID });
        } else {
          await activity.finish(`📄 ${toolResult?.file_name || "PDF එක"} Telegram එකට delivered උනා 🎉`, { chatId: CHAT_ID });
        }
        return toolResult;
      } catch (e) {
        try { await activity.fail(`❌ අසාර්ථකයි — ${e.message}`, { chatId: CHAT_ID }); } catch (_) {}
        return { error: true, message: e.message };
      } finally {
        global.__nightAgentLiveBridge = null;
      }
    }

    // Live progress: one message, edited in place, with a small spinner
    // animation while the tool runs — replaces the old pattern of sending
    // a separate "Working on..." message and a separate "Completed" message
    // (which flooded the chat with 2 messages per tool call). Uses Markdown
    // so the tool name and params render as a proper app-style card instead
    // of a flat unstyled line.
    const SPINNER_FRAMES = ["⏳", "⌛"];
    let spinnerMsgId = null;
    const toolLabel = fc.name.replace(/_/g, " ");
    const argsLine = summarizeArgs(fc.args);
    const cardBody = `*${toolLabel}*` + (argsLine && argsLine !== "{}" ? `\n\`${argsLine}\`` : "");

    try {
      const sent = await bot.sendMessage(CHAT_ID, `${SPINNER_FRAMES[0]} ${cardBody}`, { parse_mode: "Markdown" });
      spinnerMsgId = sent.message_id;
      let frame = 0;
      spinnerTimer = setInterval(() => {
        frame = (frame + 1) % SPINNER_FRAMES.length;
        bot.editMessageText(`${SPINNER_FRAMES[frame]} ${cardBody}`, { chat_id: CHAT_ID, message_id: spinnerMsgId, parse_mode: "Markdown" }).catch(() => {});
      }, 1400);
    } catch (e) {}
    ...
    try {
      const outcome = toolOutcomeTag(toolResult);
      const statusIcon = outcome.startsWith("⏸️") ? "⏸️" : outcome.startsWith("⚠️") ? "⚠️" : "✅";
      const outcomeText = outcome.replace(/^[⏸️⚠️✅]\s*/, "");
      const doneLine = `${statusIcon} ${cardBody}` + (outcomeText && outcomeText !== "✅" ? `\n_${outcomeText}_` : "");
      if (spinnerMsgId !== null) {
        await bot.editMessageText(doneLine, { chat_id: CHAT_ID, message_id: spinnerMsgId, parse_mode: "Markdown" });
      } else {
        await bot.sendMessage(CHAT_ID, doneLine, { parse_mode: "Markdown" });
      }
    } catch (e) {
      // Markdown parse failure (e.g. an odd character in a tool arg) —
      // fall back to a plain-text status so the user still sees the result.
      try {
        const outcome = toolOutcomeTag(toolResult);
        const plain = `${outcome.startsWith("⚠️") ? "⚠️" : "✅"} ${fc.name} — ${outcome.replace(/^[⏸️⚠️✅]\s*/, "") || "done"}`;
        if (spinnerMsgId !== null) await bot.editMessageText(plain, { chat_id: CHAT_ID, message_id: spinnerMsgId });
        else await bot.sendMessage(CHAT_ID, plain);
      } catch (e2) {}
    }
...
restorePendingConfirmationsFromDb();
...
bot.on("message", async (msg) => {
  ...
  if (detectedCreds.length > 0) {
    ...
    pendingConfirmations.push(credEntry);
    await persistPendingConfirmation(credEntry);
    await sendConfirmationButtons();
    return;
  }
...
bot.on("callback_query", async (query) => {
  ...
  try {
    let idx = pendingConfirmations.findIndex((pc) => pc.id === id);
    if (idx === -1) {
      // Not in this process's memory — the container may have restarted
      // since the button was shown. Check the DB before giving up; this is
      // the actual fix for "I tap the button and nothing happens."
      try {
        const { data: row } = await supabase.from("pending_confirmations").select("*").eq("id", id).maybeSingle();
        if (row) {
          pendingConfirmations.push({
            id: row.id,
            toolName: row.tool_name,
          ...
          idx = pendingConfirmations.length - 1;
        }
      } catch (e) {
        console.error("callback_query DB recovery error:", e.message);
      }
    }
    if (idx === -1) {
    ...
    await removePersistedConfirmation(pc.id);

    if (action === "cancel") {
      await bot.answerCallbackQuery(query.id, { text: "Cancelled" });
      const cancelCard = `${pc.description}\n\n❌ *Cancelled*`;
      try {
        await bot.editMessageText(cancelCard, {
          chat_id: CHAT_ID,
          message_id: query.message.message_id,
          parse_mode: "Markdown",
        });
      } catch (e) {
        await bot.editMessageText(cancelCard.replace(/[*_`]/g, ""), {
          chat_id: CHAT_ID,
          message_id: query.message.message_id,
        });
      }
    ...
    if (action === "confirm") {
      ...
      const finalCard = `${pc.description}\n\n${statusLine}`;
      try {
        await bot.editMessageText(finalCard, {
          chat_id: CHAT_ID,
          message_id: query.message.message_id,
          parse_mode: "Markdown",
        });
      } catch (e) {
        // Markdown parse failure — fall back to plain text so the update
        // still lands instead of leaving the old "waiting" card stuck.
        await bot.editMessageText(`${pc.description}\n\n${statusLine.replace(/[*_`]/g, "")}`, {
          chat_id: CHAT_ID,
          message_id: query.message.message_id,
        });
      }
...
// PDF_DELIVERY_AND_LIVE_V5_BROWSER
