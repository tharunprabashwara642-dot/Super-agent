...
const CHAT_TOOLS = [
  {
    functionDeclarations: [
      ...
      },
      {
        ...
        description: "Generates a real MCQ model-paper PDF (questions + answers) on a given topic and DELIVERS it directly to the user as a Telegram document attachment — this is the only tool that actually sends a finished PDF to the chat. Do NOT use sandbox_run to \"generate a PDF\": sandbox_run only writes files to a private throwaway disk that the user can never see or download. Use this tool instead whenever the user asks for an MCQ paper / model paper PDF.",
        parameters: {
          type: "OBJECT",
          properties: {
            topic: { type: "STRING", description: "Subject/topic for the questions, e.g. \"A/L ICT\"." },
            count: { type: "NUMBER", description: "Number of questions (1-100). Defaults to 50." },
            language: { type: "STRING", description: "Language for the questions, e.g. \"Sinhala\" or \"English\". Defaults to Sinhala." },
            title: { type: "STRING", description: "Title for the PDF. Defaults to \"<topic> MCQ Model Paper\"." },
          },
          required: ["topic"],
        },
      },
...
async function runToolDirectly(name, args) {
  ...
  if (name === "generate_mcq_pdf") {
    // Route to PDF generator based on environment variable or feature flag
    // Options:
    //   NATIVE_PDF_RENDERER=v1 (default, PDFKit + Helvetica fallback - no Sinhala)
    //   NATIVE_PDF_RENDERER=v2 (PDFKit + Noto Sans Sinhala - broken glyphs due to shaping)
    //   NATIVE_PDF_RENDERER=v3 (Puppeteer + Chrome - full Sinhala support, requires Chromium)
    const rendererVersion = process.env.NATIVE_PDF_RENDERER || 'v1';
    
    if (rendererVersion === 'v3' && process.env.ENABLE_BROWSER_PDF !== 'false') {
      try {
        return await require("./native_document_tool_v3_browser").generateDocument(args);
      } catch (browserErr) {
        console.error('[PDF] v3 browser renderer failed, falling back to v1:', browserErr.message);
        return await require("./native_document_tool_v3_browser").generateDocument(args);
      }
    } else if (rendererVersion === 'v2') {
      return await require("./native_document_tool_v3_browser").generateDocument(args);
    } else {
      return await require("./native_document_tool_v3_browser").generateDocument(args);
    }
