...
// the sub-agent's text output to the parent agent.
async function dispatchSubAgent(args = {}) {
  ...
  if (data?.error) return { error: true, message: data.error.message || "Sub-agent request failed." };
  ...
  return {
    ...
    role_name: role.name,
    emoji: role.emoji,
    result: text || "(sub-agent returned no output)",
  };
}
    ...
    if (fc.name === "generate_mcq_pdf") {
      ...
      try {
        ...
        if (toolResult && (toolResult.error || toolResult.error === true)) {
          await activity.fail(String(toolResult.message || "PDF එක හදන්න බැරි උනා"), { chatId: CHAT_ID });
        } else {
          await activity.finish("PDF එක Telegram එකට delivered උනා", { chatId: CHAT_ID });
        }
        return toolResult;
      } catch (e) {
