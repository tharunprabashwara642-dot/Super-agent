...
async function executeFunctionCall(fc, goalContext) {
  ...
  try {
    ...
    // (which flooded the chat with 2 messages per tool call).
    const SPINNER_FRAMES = ["⏳", "⌛"];
    let spinnerMsgId = null;
    const baseLine = `⚙️ ${fc.name}\n📋 ${summarizeArgs(fc.args)}`;

    try {
      const sent = await bot.sendMessage(CHAT_ID, `${SPINNER_FRAMES[0]} ${baseLine}`);
      spinnerMsgId = sent.message_id;
      let frame = 0;
      spinnerTimer = setInterval(() => {
        frame = (frame + 1) % SPINNER_FRAMES.length;
        bot.editMessageText(`${SPINNER_FRAMES[frame]} ${baseLine}`, { chat_id: CHAT_ID, message_id: spinnerMsgId }).catch(() => {});
      }, 1400);
    } catch (e) {}
    ...
    try {
      const doneLine = `✅ ${fc.name}\n📊 ${toolOutcomeTag(toolResult)}`;
      if (spinnerMsgId !== null) {
        await bot.editMessageText(formatMessageWithEmojis(doneLine), { chat_id: CHAT_ID, message_id: spinnerMsgId });
      } else {
        await bot.sendMessage(CHAT_ID, formatMessageWithEmojis(doneLine));
      }
    } catch (e) {}
