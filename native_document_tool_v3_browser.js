...
async function gemini(prompt, temperature = 0.45) {
  const pool = keys();
...
// ============================================================
// SELF-HOSTED SINHALA FONT — removes a network dependency that was the
// most likely cause of the garbled/dropped Sinhala conjuncts in generated
// PDFs. The old HTML pulled 'Noto Sans Sinhala' live from Google Fonts
// *inside* Puppeteer's own page load; any hiccup in that fetch (Railway
// egress, DNS, slow CDN, a race against page.pdf()) makes Chromium
// silently substitute a fallback font that can't shape Sinhala conjunct
// clusters (ප්‍ර, ණ්ණ, ම්පූ, etc.) correctly — the render still "succeeds",
// but glyphs are wrong or missing. `@fontsource/noto-sans-sinhala` is
// already an installed dependency (see package.json) but was never
// actually used. We read its real font files from disk once per process
// and inline them as base64 data: URIs, so the correct font is always
// available with zero network calls inside the Chromium sandbox.
// ============================================================
let _fontCssCache = null;
function buildEmbeddedFontCss() {
  if (_fontCssCache !== null) return _fontCssCache;
  try {
    const pkgJsonPath = require.resolve('@fontsource/noto-sans-sinhala/package.json');
    const pkgDir = path.dirname(pkgJsonPath);
    const indexCssPath = path.join(pkgDir, 'index.css');
    let cssSrc = fs.readFileSync(indexCssPath, 'utf8');

    // Some @fontsource releases split each weight into its own file and
    // only @import them from index.css instead of declaring @font-face
    // directly — follow those imports so we always see the real rules.
    const importPaths = [...cssSrc.matchAll(/@import\s+url\(([^)]+)\)/g)].map((m) =>
      m[1].replace(/["']/g, '').trim()
    );
    for (const imp of importPaths) {
      try {
        cssSrc += `\n${fs.readFileSync(path.resolve(pkgDir, imp), 'utf8')}`;
      } catch (_) {}
    }

    const blocks = cssSrc.match(/@font-face\s*{[^}]*}/g) || [];
    const wantedWeights = new Set(['400', '700']);
    const faces = [];
    for (const block of blocks) {
      const weightM = block.match(/font-weight:\s*(\d+)/);
      const srcM = block.match(/src:\s*url\(([^)]+)\)/);
      const rangeM = block.match(/unicode-range:\s*([^;]+);/);
      if (!weightM || !srcM || !wantedWeights.has(weightM[1])) continue;
      let bytes;
      try {
        bytes = fs.readFileSync(path.resolve(pkgDir, srcM[1].replace(/["']/g, '').trim()));
      } catch (_) {
        continue;
      }
      faces.push(
        `@font-face{font-family:'Noto Sans Sinhala';font-style:normal;font-weight:${weightM[1]};font-display:block;src:url(data:font/woff2;base64,${bytes.toString(
          'base64'
        )}) format('woff2');${rangeM ? `unicode-range:${rangeM[1].trim()};` : ''}}`
      );
    }
    _fontCssCache = faces.join('\n');
    if (!_fontCssCache) {
      console.error('⚠️ @fontsource/noto-sans-sinhala had no usable @font-face rules — falling back to Google Fonts CDN');
    }
  } catch (e) {
    console.error(`⚠️ Could not self-host Noto Sans Sinhala (${e.message}) — falling back to Google Fonts CDN`);
    _fontCssCache = '';
  }
...
// ============================================================
// SUBJECT-AWARE TOPIC BANKS — fixes "it always gives the same paper".
// The old prompt only said "about {topic}" and let Gemini free-associate;
// at a fixed low temperature it kept converging on the exact same
// partnership-into-a-company scenario with A/B/C-letter partners every
// single call. For subjects with a known, stable G.C.E. A/L syllabus we
// now hand the model a specific, rotating sub-topic per question instead
// of leaving the choice to chance.
// ============================================================
const ESSAY_TOPIC_BANKS = {
  accounting: [
    'Partnership profit/loss appropriation with interest on capital & drawings, partner salaries, and current-account (fluctuating capital) workings',
    'Admission of a new partner — goodwill treatment, asset/liability revaluation, and the new capital balances',
    'Retirement or death of a partner — revaluation account, goodwill adjustment, and settlement of the outgoing partner\u2019s balance',
    'Dissolution of a partnership — realisation account and settlement between partners (lump-sum or piecemeal distribution)',
    'Conversion of a partnership into a limited company — realisation account, purchase consideration, and the company\u2019s opening statement of financial position',
    'Limited company share issues (ordinary/preference, rights issue, or bonus issue) and the resulting statement of changes in equity',
    'Company published financial statements prepared to SLFRS/LKAS from a trial balance, with year-end adjustments (depreciation, accruals/prepayments, provision for doubtful debts)',
    'Manufacturing account and cost of production, feeding into a trading and profit or loss account',
    'Non-trading (not-for-profit) organisation accounts — receipts & payments to income & expenditure account, subscriptions in arrears/advance',
    'Incomplete records / single-entry system — statement of affairs and deriving profit from opening and closing net assets',
    'Correction of errors and a suspense account reconciling a trial balance',
    'Statement of cash flows (indirect method) prepared under LKAS 7',
    'Ratio analysis and interpretation of a company\u2019s financial statements (profitability, liquidity, efficiency, gearing)',
    'Bank reconciliation statement combined with adjustment of the cash book balance',
  ],
};
const STRUCTURED_TOPIC_BANKS = ESSAY_TOPIC_BANKS;

function normalizeSubjectKey(subject) {
  const s = String(subject || '').toLowerCase();
  if (/account|ගිණුම්|ලේඛන|ගණකාධ/.test(s)) return 'accounting';
  return s.replace(/[^a-z]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 40) || 'general';
}

// Recently-used sub-topics per subject, kept for this process's lifetime so
// back-to-back requests (the normal Telegram-bot usage pattern) don't repeat
// the same scenario across separate PDFs, not just within one PDF.
const _recentTopics = new Map(); // subjectKey -> string[]

function pickTopics(subjectKey, bank, need) {
  if (!bank || !bank.length) return [];
  const recent = _recentTopics.get(subjectKey) || [];
  const fresh = bank.filter((t) => !recent.includes(t));
  const pool = (fresh.length >= need ? fresh : bank).slice();
...
function markTopicsUsed(subjectKey, topics) {
  if (!topics.length) return;
  const recent = _recentTopics.get(subjectKey) || [];
  _recentTopics.set(subjectKey, [...topics, ...recent].slice(0, 24));
}

// ============================================================
// RESEARCH GROUNDING — best-effort, optional (needs BRAVE_API_KEY, the
// same key index.js's own web_search tool uses). Grounds the writer
// prompt in how real Sri Lankan A/L papers are actually phrased/structured
// instead of relying purely on the model's own guess. Degrades silently
// to '' if no key is configured or the request fails — never blocks
// generation.
// ============================================================
async function braveSnippet(query) {
  ...
  try {
    ...
    return items.slice(0, 3).map((it) => `- ${cleanText(it.title || '')}: ${cleanText(it.description || '')}`).join('\n');
  } catch (_) {
    return '';
  }
}
...
// ============================================================
// VERIFIER SUB-AGENT — cross-checks each generated question before it is
// accepted, instead of trusting the writer model's first draft blindly.
// This is the "sub-agents talking to each other" step: it calls Claude via
// anthropic_brain.js (a different model family than the Gemini writer, so
// the same blind spot doesn't grade its own homework) when an Anthropic
// key is configured, and falls back to a second, stricter Gemini pass
// otherwise. If neither is usable, it doesn't block generation.
// ============================================================
let _anthropicBrain = null;
function anthropicBrain() {
  if (_anthropicBrain === null) {
    try {
      _anthropicBrain = require('./anthropic_brain');
    } catch (_) {
      _anthropicBrain = false;
    }
  }
  return _anthropicBrain || null;
}

async function verifyCandidate(subject, language, kind, candidateSummary) {
  const prompt = `You are a strict verification sub-agent checking ONE exam question before it is printed in a Sri Lankan G.C.E. Advanced Level ${subject} ${kind} model paper (language: ${language}).

CANDIDATE:
${candidateSummary}

Check ALL of:
1. The numbers/scenario are internally consistent and the question is actually answerable (figures add up, ratios are computable, no contradictions).
2. Any listed mark allocations sum exactly to the stated total.
3. The wording uses correct, standard terminology for the subject and reads like an authentic Sri Lankan A/L exam question, not an awkward translation.
4. The scenario is not a generic, overused textbook cliché unless that IS the assigned topic.

Return ONLY this JSON, no markdown fences:
{"ok": true|false, "reason": "short reason if false, empty string if true"}`;

  const brain = anthropicBrain();
  if (brain && brain.chatShimmed) {
    try {
      const resp = await brain.chatShimmed(
        [{ role: 'user', parts: [{ text: prompt }] }],
        'You are a meticulous exam-paper verifier. Reply with strict JSON only.',
        null,
        undefined,
        30000
      );
  ...
  try {
    const text = await gemini(prompt, 0.1);
    const parsed = JSON.parse(cleanJson(text));
    if (typeof parsed.ok === 'boolean') return parsed;
  } catch (_) {}
  // Verification itself unavailable (no keys) — don't block generation.
...
async function makeStructuredQuestions(subject, topic, count, language) {
  ...
  const bank = STRUCTURED_TOPIC_BANKS[subjectKey];
  const research = bank
    ? await braveSnippet(`${subject} G.C.E. Advanced Level Sri Lanka structured exam question style ${topic}`)
    : '';
  ...
  for (let round = 0; out.length < count && round < 15; round++) {
    ...
    const roundTopics = pickTopics(subjectKey, bank, need);
    const topicInstruction = roundTopics.length
      ? `Base each of the ${need} questions on a DIFFERENT one of these exact sub-topics, in this order:\n${roundTopics
          .map((t, i) => `${i + 1}. ${t}`)
          .join('\n')}`
      : `Cover ${need} genuinely different sub-topics within "${topic}" — do not repeat the same scenario shape twice.`;
    const raw = await gemini(
...
Use varied, realistic entity/person names and varied, non-round numeric figures each time — do not reuse the same names or figures across questions. Make sure every figure given is internally consistent and the question is actually solvable.
${research ? `For style/structure grounding only (do not copy wording verbatim):\n${research}\n` : ''}Return ONLY valid JSON (no markdown, no code fences):
...
${existing || '(none)'}`,
    ...
    for (const q of parsed.questions || []) {
      ...
      const partsSum = parts.reduce((s, p) => s + p.marks, 0);
      if (question && parts.length >= 2 && Math.abs(partsSum - marks) <= 1) {
        const summary = `Question: ${question}\nTotal marks: ${marks}\nParts:\n${parts
          .map((p) => `${p.label} ${p.text} [${p.marks}] — answer: ${p.answer}`)
          .join('\n')}`;
        const verdict = await verifyCandidate(subject, language, 'structured', summary);
        if (verdict.ok) {
          out.push({ question, marks, parts });
        } else {
          console.error(`⚠️ Structured candidate rejected by verifier: ${verdict.reason || 'unspecified'}`);
        }
      }
      if (out.length >= count) break;
    }
    if (roundTopics.length) markTopicsUsed(subjectKey, roundTopics);
  }
  if (out.length < count) {
  ...
  const blocks = questions
    .map((q, i) => {
      ...
      <div class="question-block">
        <div class="question-text">
          <div class="q-head">
            <span><span class="question-number">${i + 1}.</span>${escapeHtml(q.question)}</span>
...
async function makeEssayQuestions(subject, topic, count, language) {
  const subjectKey = normalizeSubjectKey(subject);
  const bank = ESSAY_TOPIC_BANKS[subjectKey];
  const research = bank
    ? await braveSnippet(`${subject} G.C.E. Advanced Level Sri Lanka essay exam question style ${topic}`)
    : '';
  ...
  for (let round = 0; out.length < count && round < 15; round++) {
    ...
    const topicInstruction = roundTopics.length
      ? `Base each of the ${need} questions on a DIFFERENT one of these exact sub-topics, in this order:\n${roundTopics
          .map((t, i) => `${i + 1}. ${t}`)
          .join('\n')}\nDo NOT default to a generic partnership-to-company example unless it is the assigned sub-topic above.`
      : `Cover ${need} genuinely different sub-topics within the "${topic}" syllabus area — do not repeat the same scenario shape twice.`;
    const raw = await gemini(
...
Use varied, realistic entity/person names each time (not always the same letters like A, B, C) and varied, non-round numeric figures — never reuse the same names or figures across questions. Make sure every number given is internally consistent (assets = liabilities + capital, ratios given are actually computable, percentages/amounts reconcile, etc.) so the question is genuinely answerable.
${research ? `For style/structure grounding only, do not copy wording verbatim:\n${research}\n` : ''}Return ONLY valid JSON (no markdown, no code fences):
...
${existing || '(none)'}`,
    ...
    for (const q of parsed.questions || []) {
      ...
      const marksSum = guidance.reduce((s, g) => s + g.marks, 0);
      if (question && guidance.length >= 3 && Math.abs(marksSum - marks) <= 2) {
        const summary = `Question: ${question}\nTotal marks: ${marks}\nGuidance:\n${guidance
          .map((g) => `- ${g.point} [${g.marks}]`)
          .join('\n')}`;
        const verdict = await verifyCandidate(subject, language, 'essay', summary);
        if (verdict.ok) {
          out.push({ question, marks, guidance });
        } else {
          console.error(`⚠️ Essay candidate rejected by verifier: ${verdict.reason || 'unspecified'}`);
        }
      }
      if (out.length >= count) break;
    }
...
const STRUCTURED_EXTRA_CSS = `
    ...
    .q-head { display: flex; justify-content: space-between; align-items: baseline; gap: 10pt; }
    .total-marks { flex-shrink: 0; color: #64748b; font-weight: 600; font-size: 9pt; white-space: nowrap; }
...
function wrapHtml(title, subject, subheading, bodyBlocks, tailSection, extraCss = '') {
  const embeddedFont = buildEmbeddedFontCss();
  // Self-hosted font (see buildEmbeddedFontCss) removes the network
  // round-trip that was silently dropping/garbling Sinhala conjuncts.
  // Falls back to the live CDN link only if the local font package
  // couldn't be read for some reason, so this never hard-fails a PDF.
  const fontLinkTag = embeddedFont
    ? ''
...
async function generatePdfViaHtml(html) {
  ...
  try {
    ...
    const pdfBuffer = await page.pdf({
      ...
      timeout: 45000,
      tagged: true, // accessible/tagged PDF export — more reliable Unicode text mapping for complex scripts
    });
